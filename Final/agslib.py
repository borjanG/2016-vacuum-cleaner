...
class Population(object):
   ...
   # Quel croisement 1 point, 2 points ou uniforme
    ...
    def __get_best(self):
        return self.__bestIndividu,self.__bestAdequation,self.__iteration
    def __set_best(self,v):
        assert isinstance(v,Individu)
        # utilisé pour traquer le bug mmcPop
        # if hasattr(self,'target'):
        #     print("best is",v.genotype,v.adequation,'should be',self.target)
        self.__bestIndividu = v
        self.__bestAdequation = v.adequation
        self.__iteration = self.age
    def __del_best(self):
        self.__bestIndividu,self.__bestAdequation,self.__iteration = None,0,-1
    ...
    def run(self,nbIterations,fichier=None,code=0,verbose=False):
        """
        ...
        if fichier == None : # pas de fichier de sauvegarde
            return bestIndividu.genotype
        else: # fichier de sauvegarde
            with open(fichier,'w') as f : print(bestIndividu.genotype,file=f)

        """

        self.restart()
        self.best = _c = max(self.popAG)
        self.history[self.age] = self.fitvalues,(_c,_c.adequation)
        if verbose: print("méthode de sélection: %s" % self.select(code))
        while self.age < nbIterations and not self.stable:
            # permet de voir que tout va bien
            if float(sys.version[:3]) >= 3.3 and verbose:
                print('.',end='',file=sys.stderr,flush=True)
            self.age += 1
            self.popAG = self.nextGeneration(code)
            _c = max(self.popAG)
            self.history[self.age] = self.fitvalues,(_c,_c.adequation)
            if self.bestEval < _c.adequation  :
                self.best = _c
        # Detection de la cause de sortie
        if verbose:
            if self.stable:
                print('stabilisation iteration',self.age)
                print("most representative eval is : ",
                      self.mostRepresentative.adequation)
            print("max obtenu à l'iteration {0.quand} adéquation {0.bestEval}"
                  .format(self))

            for i in (0,self.age):
                print("{} : {}".format(i,self.history[i]))
        if fichier is not None : # fichier de sauvegarde
            with open(fichier,'w') as f :
                print(self.bestIndividu.genotype,file=f)

        return self.bestIndividu.genotype
    ...
    def isOver(self):
        ...
        facultatif
        _ng : nombre de gènes ayant convergé
        """
        _ng,i = 0,0
        _ok = True
        while i < self.nbGenes :
            _ok = self.hasConverged(i)
            if _ok : _ng += 1
            i += 1
        if self.rateCVG * self.nbGenes <= _ng: _ok = True
        else: _ok = False
    ...
    def hasConverged(self,numGene):
        ...
        _istart = numGene * self.szGenes
        _ifinal = _istart + self.szGenes
        for x in self.popAG:
            _sub = x.genotype[_istart:_ifinal]
            _dGene[_sub] = _dGene.get(_sub,0) + 1

        _frep,_taux = False, self.rateCVG*self.szPop
        for gene in _dGene :
            if _dGene[gene] >= _taux:
                _frep = True
                break # on a trouvé un gène convergent
        return _frep
    ...
    def _selectFraction(self,nbParents=None):
        ...
        for i in range(self.szPop):
            x = self.popAG[i]
            a,b = math.modf( getattr(x,_attr) / _moy )
            _pop.extend( [x for _ in range( int(b) ) ] )
            if a > 0 : _residu.append(x)
        _missing = self.szPop - len(_pop)
        try:
            _pop.extend(random.sample(_residu,_missing))
        except:
            # probleme a identifier
            print("size",self.szPop)
            print("pop",_pop)
            print("residu",_residu)
            print("manque",_missing)
            
        for x in _pop : x.reset()
        if nbParents is None  or nbParents == self.szPop :
            random.shuffle(_pop)
            return _pop
        
        return random.sample(_pop,nbParents)
