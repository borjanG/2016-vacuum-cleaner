...
class GeneratePercept(object):
    ...
    def producer(self):
        ...
        la production doit toujours etre dans le meme ordre !!
        """
        import itertools
        _dom1 = sorted([x for x in self.dObj.keys() if 0 <= x < 100]) # clefs valides
        _dom2 = _dom1[:] + [-1] # domaine pour capteur in 0..7
        _support = [ _dom1 if _ == 8 else _dom2 for _ in self.lCap ]
